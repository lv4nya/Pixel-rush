...
for (let y = 0; y < height; y++) {
  for (let x = 0; x < width; x++) {
    ...
    while (queue.length > 0) {
      ...
      const coffset = (cy * width + cx) * bytesPerPixel;
      sumR += pixels[coffset];
      sumG += pixels[coffset + 1];
      sumB += pixels[coffset + 2];
      ...
      for (const [nx, ny] of neighbors) {
        if (nx >= 0 && nx < width && ny >= 0 && ny < height) {
          ...
          if (!visited[nidx]) {
            ...
            const nr = pixels[noffset], ng = pixels[noffset + 1], nb = pixels[noffset + 2];
...
boxes.forEach((box, i) => {
  const hex = '#' + ((1 << 24) + (box.avgColor.r << 16) + (box.avgColor.g << 8) + box.avgColor.b).toString(16).slice(1);
  console.log(`Sprite ${i + 1}: x=${box.minX}, y=${box.minY}, w=${box.width}, h=${box.height} -> avgColor = RGB(${box.avgColor.r}, ${box.avgColor.g}, ${box.avgColor.b}) (${hex})`);
});
